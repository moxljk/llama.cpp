...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_block_zipserv = [&](const LLM_TN_IMPL & tn) -> llama_block_zipserv {
            const LLM_BN_ZIPSERV_IMPL bn(tn);
            const std::string base_name = bn.base.str();
            const auto meta = llama_zipserv_load_meta(ml, base_name, true);

            auto create_payload = [&](const char * field, uint32_t ne) -> ggml_tensor * {
                GGML_ASSERT(tn.suffix != nullptr);
                const std::string suffix = format("%s.%s", tn.suffix, field);
                const LLM_TN_IMPL payload_tn(tn.arch, tn.tensor, suffix.c_str(), tn.bid, tn.xid);
                return create_tensor(payload_tn, { static_cast<int64_t>(ne) }, 0);
            };

            llama_block_zipserv block;
            // Zipserv payload tensors are required parts of the compressed weight block.
            block.sign_mantissa       = create_payload("sign_mantissa",       meta.sign_mantissa_size);
            block.compressed_full     = create_payload("compressed_full",     meta.compressed_full_size);
            block.bitmap1             = create_payload("bitmap1",             meta.bitmap1_size);
            block.bitmap2             = create_payload("bitmap2",             meta.bitmap2_size);
            block.bitmap3             = create_payload("bitmap3",             meta.bitmap3_size);
            block.tile_offsets_median = create_payload("tile_offsets_median", meta.tile_offsets_median_size);
            block.tile_offsets_global = create_payload("tile_offsets_global", meta.tile_offsets_global_size);
            block.rows                = meta.rows;
            block.cols                = meta.cols;
            block.max_high_freq_count = meta.max_high_freq_count;
            block.max_full_count      = meta.max_full_count;
            block.start_exp           = meta.start_exp;
            ...
            case LLM_ARCH_QWEN3ZIP:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.wq_zipserv       = create_block_zipserv(tn(LLM_TENSOR_ATTN_Q_ZIPSERV,   "weight", i));
                        layer.wk_zipserv       = create_block_zipserv(tn(LLM_TENSOR_ATTN_K_ZIPSERV,   "weight", i));
                        layer.wv_zipserv       = create_block_zipserv(tn(LLM_TENSOR_ATTN_V_ZIPSERV,   "weight", i));
                        layer.wo_zipserv       = create_block_zipserv(tn(LLM_TENSOR_ATTN_OUT_ZIPSERV, "weight", i));
                        
                        layer.attn_k_norm = create_tensor(tn(LLM_TENSOR_ATTN_K_NORM, "weight", i), {n_embd_head_k}, 0);
                        layer.attn_q_norm = create_tensor(tn(LLM_TENSOR_ATTN_Q_NORM, "weight", i), {n_embd_head_k}, 0);

                        layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd}, 0);
                        layer.ffn_gate_zipserv = create_block_zipserv(tn(LLM_TENSOR_FFN_GATE_ZIPSERV, "weight", i));
                        layer.ffn_down_zipserv = create_block_zipserv(tn(LLM_TENSOR_FFN_DOWN_ZIPSERV, "weight", i));
                        layer.ffn_up_zipserv   = create_block_zipserv(tn(LLM_TENSOR_FFN_UP_ZIPSERV,   "weight", i));
                    }
                } break;
            case LLM_ARCH_QWEN3:
            case LLM_ARCH_QWEN3VL:
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_QWEN3ZIP:
            {
                llm = std::make_unique<llm_build_qwen3zip>(*this, params);
            } break;
        case LLM_ARCH_QWEN3MOE:
            {
