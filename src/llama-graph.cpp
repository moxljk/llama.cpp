...
void llm_graph_input_cls::set_input(const llama_ubatch * ubatch) {
    ...
    if (cparams.embeddings && (
        ...
        const bool last = (
             cparams.pooling_type == LLAMA_POOLING_TYPE_LAST ||
            (cparams.pooling_type == LLAMA_POOLING_TYPE_RANK && (arch == LLM_ARCH_QWEN3 || arch == LLM_ARCH_QWEN3VL || arch == LLM_ARCH_QWEN3ZIP)) // qwen3 reranking & embedding models use last token
        );
        ...
        case LLAMA_POOLING_TYPE_RANK:
            {
                ...
                if (arch == LLM_ARCH_QWEN3 || arch == LLM_ARCH_QWEN3VL || arch == LLM_ARCH_QWEN3ZIP) {
